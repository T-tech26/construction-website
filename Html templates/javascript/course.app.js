...
const displayData = (data) => {
  ...
  document.getElementById('info-course').textContent = data.course;
...
const submitForm = (alertMsg, data) => {
  const serverUrl = '/training.course.application.page';
  fetch(
    serverUrl,
    {
      method: 'POST',
...
const validateForm = (formData) => {
  const { name, address, gender, DOB, nationality, state, course, number, email, terms } = formData;
  ...
  if(!name || name.length < 3) {
    ...
    alertMsg.textContent = 'Please choose a course';
    alertMsg.style.animationName = 'show-res-msg';
    window.scrollTo({top: 0, behavior: 'smooth'})
    hideMsg(alertMsg);
